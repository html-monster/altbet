...
namespace AltBet.ExchangeServices.Services
{
    ...
    public class PaymentService : IPaymentManager
    {
        ...
        public SkrillOutResponse SkrillOut(SkrillOutRequest model)
        {
            ...
            var currentBalance = CommonManager.Server.GetUserBalanceRequestMethod(new GetUserBalanceRequest { UserName = model.Name });
            ...
            if (result.status == "2" || result.status == "1")
            {
                ...
                //PaymentService paymentManager = new PaymentService();
                ...
                bool addFirstStepHistory = InsertHistory(result.amount, "Skrill", DateTime.Now, "Withdraw", model.Name, result.transactionId, "Processed");
        ...
        public Payment NetellerOut(TransferOut model)
        {
            ...
            var currentBalance = CommonManager.Server.GetUserBalanceRequestMethod(new GetUserBalanceRequest { UserName = model.name });
                //balance.GetCurrentUserAssets(model.name);
